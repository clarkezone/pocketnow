...
var rootCmd = &cobra.Command{
	Use:   "geocache",
	Short: "geocache is a URL shortener.",
	Long: `geocache is a URL shortener based on the pocketbase backend.
...
Then run the following:
  sudo -- sh -c 'geocache completion zsh > /usr/local/share/zsh/site-functions/_geocache'
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.geocache.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".geocache" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigType("yaml")
		viper.SetConfigName(".geocache")
	}
